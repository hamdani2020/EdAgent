...
- User preference caching and management
...
def safe_parse_datetime(datetime_str: Optional[str]) -> Optional[datetime]:
    ...
    try:
        ...
        get_logger().warning(f"Failed to parse datetime string '{datetime_str}': {e}")
        return None
...
class SessionManager:
    """
    Enhanced session management system with security, persistence, and validation
    """
    ...
    def __init__(self):
        ...
        self.session_timeout = timedelta(minutes=self.config.security.session_timeout_minutes)
        self.token_refresh_threshold = timedelta(minutes=self.config.security.token_refresh_threshold_minutes)
        
        # Security settings
        self.encrypt_session_data = self.config.security.encrypt_session_data
        self.max_session_age = timedelta(days=30)  # Maximum session age
    ...
    def _initialize_session_state(self) -> None:
        ...
        default_keys = {
            ...
            f"{self.session_key_prefix}navigation_state": {
                "current_tab": "chat", 
                "previous_tab": None,
                "breadcrumb": []
            },
            ...
            f"{self.session_key_prefix}security_state": {
                "login_attempts": 0,
        ...
        # Mark as initialized and update activity
    ...
    def _generate_session_fingerprint(self) -> None:
        """Generate session fingerprint for security validation"""
        # Create a simple fingerprint based on session creation time and user agent
        # In production, this could include more sophisticated fingerprinting
        session_created = st.session_state.get(self._get_session_key("session_created_at"))
        if not session_created:
            session_created = datetime.now()
            st.session_state[self._get_session_key("session_created_at")] = session_created
        
        # Simple fingerprint using session creation time
        fingerprint_data = f"{session_created.isoformat()}"
        fingerprint = hashlib.sha256(fingerprint_data.encode()).hexdigest()[:16]
        
        security_state = st.session_state.get(self._get_session_key("security_state"), {})
        security_state["session_fingerprint"] = fingerprint
        st.session_state[self._get_session_key("security_state")] = security_state
    
    def _encrypt_data(self, data: str) -> str:
        """Simple data encryption for session storage"""
        if not self.encrypt_session_data or not data:
            return data
        
        # Simple base64 encoding for basic obfuscation
        # In production, use proper encryption with keys
        try:
    ...
    def _decrypt_data(self, encrypted_data: str) -> str:
        ...
        if not self.encrypt_session_data or not encrypted_data:
    ...
    def is_authenticated(self) -> bool:
        ...
        if not self._validate_session_integrity():
            self.logger.warning("Session integrity validation failed")
            return False
    ...
    def get_session_state(self) -> SessionState:
        """Get current session state with comprehensive checks"""
        user_info = st.session_state.get(self._get_session_key("user_info"))
        access_token = st.session_state.get(self._get_session_key("access_token"))
    ...
    def set_auth_data(
        ...
        """Store authentication data in session with security measures"""
        try:
            # Store user info
            ...
            st.session_state[self._get_session_key("access_token")] = self._encrypt_data(access_token)
            if refresh_token:
                st.session_state[self._get_session_key("refresh_token")] = self._encrypt_data(refresh_token)
            ...
            user_id = user_info.user_id if hasattr(user_info, 'user_id') else user_info.get('user_id', 'unknown')
            self.logger.info(f"Authentication data stored for user {user_id}")
            
        except Exception as e:
            self.logger.error(f"Failed to store authentication data: {e}")
            raise
    ...
    def clear_session(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Clearing session for user {user_id}")
            ...
            self.logger.info("Session data cleared and reinitialized")
            
        except Exception as e:
            self.logger.error(f"Failed to clear session: {e}")
            # Force clear all session state as fallback
    ...
    def _validate_session_integrity(self) -> bool:
        ...
        try:
            ...
            security_state = st.session_state.get(self._get_session_key("security_state"), {})
            if not security_state.get("session_fingerprint"):
                self.logger.warning("Missing session fingerprint")
                return False
            ...
            if session_created:
                ...
                if datetime.now() - session_created > self.max_session_age:
                    self.logger.warning("Session too old")
                    return False
            ...
            if user_info and not access_token:
                self.logger.warning("User info exists but no access token")
                return False
            
            if access_token and not user_info:
                self.logger.warning("Access token exists but no user info")
                return False
    ...
    def _is_account_locked(self) -> bool:
        ...
        security_state = st.session_state.get(self._get_session_key("security_state"), {})
        locked_until = security_state.get("locked_until")
        
        if locked_until:
            if isinstance(locked_until, str):
                locked_until = safe_parse_datetime(locked_until)
    ...
    def record_failed_login_attempt(self) -> None:
        """Record failed login attempt and handle account locking"""
        security_state = st.session_state.get(self._get_session_key("security_state"), {})
        
        # Increment attempt counter
        attempts = security_state.get("login_attempts", 0) + 1
        security_state["login_attempts"] = attempts
        security_state["last_login_attempt"] = datetime.now()
        ...
        if attempts >= self.config.security.max_login_attempts:
            ...
            security_state["locked_until"] = datetime.now() + lockout_duration
            
            self.logger.warning(f"Account locked after {attempts} failed login attempts")
        
        st.session_state[self._get_session_key("security_state")] = security_state
    
    def get_remaining_lockout_time(self) -> Optional[timedelta]:
        """Get remaining lockout time if account is locked"""
        security_state = st.session_state.get(self._get_session_key("security_state"), {})
        locked_until = security_state.get("locked_until")
    ...
    def handle_auth_error(self) -> None:
        ...
        self.logger.warning("Authentication error occurred, clearing session")
        
        # Record the error for monitoring
        user_info = self.get_current_user()
        if user_info:
            self.logger.warning(f"Auth error for user {user_info.user_id}")
    ...
    def set_current_tab(self, tab: str) -> None:
        ...
        # Update breadcrumb
        breadcrumb = nav_state.get("breadcrumb", [])
        if not breadcrumb or breadcrumb[-1] != tab:
            breadcrumb.append(tab)
            # Keep breadcrumb to reasonable length
            if len(breadcrumb) > 5:
                breadcrumb = breadcrumb[-5:]
            nav_state["breadcrumb"] = breadcrumb
    ...
    def get_navigation_breadcrumb(self) -> List[str]:
        ...
        nav_state = st.session_state.get(self._get_session_key("navigation_state"), {})
        return nav_state.get("breadcrumb", [])
    ...
    def add_notification(self, message: str, type: str = "info", duration: int = 5) -> None:
        ...
        notifications = self.get_ui_state("notifications", [])
        notification = {
            "message": message,
            "type": type,
        ...
        self.set_ui_state("notifications", notifications)
    
    def get_notifications(self) -> List[Dict[str, Any]]:
        """Get current notifications"""
        return self.get_ui_state("notifications", [])
    
    def clear_notifications(self) -> None:
        """Clear all notifications"""
        self.set_ui_state("notifications", [])
    ...
    def set_cached_data(self, cache_key: str, value: Any, ttl_hours: int = 1) -> None:
        ...
        try:
            ...
            st.session_state[self._get_session_key("cached_data")] = cached_data
            self.logger.debug(f"Cached data set for key: {cache_key}")
            
        except Exception as e:
            self.logger.error(f"Failed to set cached data: {e}")
    
    def clear_cached_data(self, cache_key: Optional[str] = None) -> None:
        """Clear cached data"""
        try:
            cached_data = st.session_state.get(self._get_session_key("cached_data"), {})
            ...
            if cache_key:
                if cache_key in cached_data:
                    del cached_data[cache_key]
                    self.logger.info(f"Cleared cache key: {cache_key}")
            else:
                cached_data.clear()
            ...
            self.logger.error(f"Failed to clear cached data: {e}")
    ...
    def load_session_state(self) -> None:
        ...
        try:
            ...
            if not self._validate_session_integrity():
                self.logger.info("Invalid session detected during load, clearing")
                self.clear_session()
                return
            ...
            self.logger.debug("Session state loaded and validated")
    ...
    def get_session_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "breadcrumb": self.get_navigation_breadcrumb(),
            "cached_data_keys": list(st.session_state.get(self._get_session_key("cached_data"), {}).keys()),
            "login_attempts": security_state.get("login_attempts", 0),
            "is_locked": self._is_account_locked(),
            "session_fingerprint": security_state.get("session_fingerprint")
        }
